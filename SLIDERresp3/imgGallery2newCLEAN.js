...
function thumbnails(imgsarray) {
    ...
    //get image orientation, and set class
        //create img object
        let img = document.createElement('img');
        img.src = `${myImages[index]}`;   //gets src from array index
        //pass img to function to get orientation
        let orientation = getImgOrientReturnClass(img);
        let orientationthumb;
        //set css class, depending on result
        if(orientation === 'portrait') {
            orientationthumb = 'thumb-portrait';
        }
        else if (orientation === 'landscape') {
            orientationthumb = 'thumb-landscape'
        }
    let imgid = 'thumb' + index; //create new imgid 
    let slideDynamicNew  = `<img id="${imgid}" class="thumb ${orientationthumb}" src="${myImages[index]}">`; //create img with template literal
...
const allimages = () => {
    ...
    myImages.forEach((element, index) => {
        ...
        const startdelay = setTimeout(() => {
            ...
            const parentdiv = document.querySelector('#mainphotodiv2'); //create div element to append the image to
            ...
            if(availablewidth < cssmaxwidth) {
                var height = availablewidth * 0.677;
                var width = availablewidth;
                if(orientation == 'portrait') {
                    var height = availablewidth * 0.677;
                    var width = height * 0.677;
                ...
                if(availablewidth >= cssmaxwidth) {
                    ...
                    var width = availablewidth;
                    var height =  availablewidth * 0.67 - 100;
                }
                if(orientation == 'portrait') {
                    var height = width * 0.667 -100;
